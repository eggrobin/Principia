...
ComputeAngularDegreesOfFreedom(
  ...
  Vector<double, InertialFrame> const local_north = Normalize(
      secondary_->polar_axis().OrthogonalizationAgainst(line_of_sight));
  ...
  Displacement<InertialFrame> parallel_radius =
      line_of_sight.OrthogonalizationAgainst(secondary_->polar_axis());
